...
\begin{figure}[H]
\begin{center}
	\includegraphics[scale=.45]{images/abstract.png}
\end{center}
...
\begin{figure}[H]
\begin{center}
	\includegraphics[scale=.15]{images/About.png}
\end{center}
...
\begin{abstract}
\underline{Task:}
The task of this diploma thesis is to create an administration program and a new website for the company Augenoptik Aigner in Wels. \newline The website has to be mobile responsive. Besides the location and the opening hours, the various eyewear models should be showed on the website. The user can use the website to inform the seller about his interests.The seller should be able to log into the website from his home, to check or edit his spreadsheets, like for example to add new glasses. The website should be appealing to customers and provide a good overview of the eyewear models. In addition, the administrator should be able to manage his eyewear models from home.
\newline The administration program should be implemented as a desktop application which should completely replace the previously used program. The software is intended to manage all customer, order, supplier and eyeglass frame data. In addition Word documents such as invoices should be exportable and the user should be able to see statistics of his sold glasses and contact lenses. Furthermore it should also be able to send emails and SMS from the program.
\newline \underline{Realization:}
The current website is not mobile responsive and the only features are to contact the seller and to check general data.The website could have been implemented in different programming languages. It was developed on the basis of previous knowledge using Asp .net. The Mvc pattern was implemented. \newline The currently used administration software is an obsolete Microsoft DOS program which is inconvenient and time consuming to use. To implement the new version WPF and the MVVM pattern (applied with MVVM-Light) were used. 
\newline \underline{Results:}
It is a finished mobile responsvie website that will be provided to the company $"$Augenoptik Aigner".
 \newline The result of the administration program is a finished software product which should soon be implemented at the company Augenoptik Aigner. The software can be examined in the enclosed CD.
\end{abstract}
\end{otherlanguage}
\newpage \newpage
\section*{Danksagung}
